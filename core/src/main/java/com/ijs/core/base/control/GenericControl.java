...
public class GenericControl extends BaseControl{
	...
	public @ResponseBody PageList list(@PathVariable("pn")Integer pn,@PathVariable("modelkey") String modelkey,NativeWebRequest request){
		PageList list=new PageList();
		try {
			log.debug("execute the list of genricControl");
			BaseModel obj = (BaseModel) dataBinder(request, false,modelkey);
			list.setPerPageSize(obj.getQp().getCount());
			list.setCurrentPage(pn);
			genericServ.list(obj,list);
		} catch (Exception e) {
			e.printStackTrace();
