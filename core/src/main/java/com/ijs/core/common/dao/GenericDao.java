...
public class GenericDao {
			...
			for (Field field : fields) {
				try {
					...
					if (entity != null) {
						...
						Field[] fs = t.getSuperclass().getDeclaredFields();
						if(fs.length == 0){
						    fs = t.getDeclaredFields();
                        }
						Field.setAccessible(fs, true);
						for (Field f : fs) {
							if (f.getName().equals((field.getName() + "Id"))) {
								if (f.get(obj) != null) {
									field.set(obj, this.em.find(field.getType(), f.get(obj)));
								}
							}
						}
	 ...
	 * @throws Exception
	 */
	public <T> String generateJpql(T t,StringBuffer sb,PropertyDescriptor previous,Boolean noLike) throws Exception {
		...
		if(sb==null) {
			...
			for(Constructor<?> cons:conses) {
				...
				if(dcfl!=null) {
					...
					if(paras.length>0) {
						sb.append("select ");
						sb.append(" new ").append(t.getClass().getName()).append("(");
						for(int i=0;i<paras.length;i++) {
							String[] colName=paras[i].split("_");
							if(colName.length<2) {
								sb.append(alias).append(".").append(colName[0]);
							}else {
								...
								sb.append(colName[0]).append(".").append(colName[1]);
			...
			sb.append(" from " + t.getClass().getSimpleName()).append(" ").append(alias).append(" where 1=1 ");
		}else {
			sb.insert(sb.indexOf("where")-1, ","+t.getClass().getSimpleName()+" "+alias);
			sb.append(" and ").append(alias).append(".id").append("=").append(previous.getValue("alias")).append(".").append(previous.getName());
		}
		...
		if(orderField!=null&&previous==null) {
			sb.append(" order by ").append(alias).append(".").append(orderField.getName()).append(" ").append(orderField.getAnnotation(OrderBy.class).value());
		}
		return sb.toString();
	}
	/**
	...
	public <T> List<T> findByEntity(T t) {
		try {
			return (List<T>) find(generateJpql(t, null,null,null));
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public <T> List<T> findByEntity(T t, boolean noLike) {
		try {
			return (List<T>) find(generateJpql(t, null, null,noLike));
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
