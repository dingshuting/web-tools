...
public class ExtraDataServ extends  MysqlBaseServiceListener{
	...
	public boolean doAction(ACTION action,Class t, Object... args) throws ServiceException {
		...
		if(args.length>0&&args[0] instanceof ExtraData){
			switch (action) {
			case AFTER_SAVE:
	...
	private void saveFunc(ExtraData ed){
		...
		//新增按钮	
			...
			/*//查询所有数据
			Func allListFunc = new Func();
			allListFunc.setId(UUID.randomUUID().toString());
			allListFunc.setParentId(func.getId());
			allListFunc.setTypes(1);
			allListFunc.setName("所有数据");
			allListFunc.setLevel(3);
			allListFunc.setDisplayType(3);
			allListFunc.setTogo("allList");
			allListFunc.setStatus(1);
			allListFunc.setIcon("");
			allListFunc.setFnOr(1);
			allListFunc.setFnDesc("1");
			allListFunc.setUrl("/"+ed.getNameCodeAlias()+"/all");
			allListFunc.setModelClass("");
			allListFunc.setExtraDataId(ed.getId());
			dao.save(allListFunc);*/
	}
