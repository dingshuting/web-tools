...
steal(
	...
	function(can,ViewFactory) {
		steal('js/jquery.showLoading.js','css/showloading.css');
		Modal=can.Construct.extend(
				{
					...
					confirm:function(title,body,onconfirm,oncancle){
							var html=body;
                            //new $.flavr({dialog : 'confirm', form : {content: html}, onConfirm : onconfirm });
                            $$$.confirm(body,title,onconfirm);
                           /* if(confirm(body)){
                            	onconfirm();
                            }else{
                            	if(oncancle)
					...
					confirmList:function(title,body,data,onconfirm,oncancle){
							...
							ViewFactory.build(body,html,data);
...
//						$.AMUI.progress.start();
					...
					prompt:function(body,onConfirm,onCancel){
						new $.flavr({ content : body,
                            dialog : 'confirm', onConfirm : onConfirm, onCancel : onCancel}); 
					},
